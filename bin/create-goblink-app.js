...
// --- Scaffold functions ---
function scaffoldNextjs(dir, mode) {
    writeFile(path.join(dir, ".env.local.example"), envTemplate(mode));
    writeFile(path.join(dir, "app", "api", "payments", "route.ts"), nextjsPaymentRoute());
    writeFile(path.join(dir, "app", "api", "webhooks", "goblink", "route.ts"), nextjsWebhookRoute());
...
function scaffoldExpress(dir, mode) {
...
function scaffoldPlain(dir, mode) {
    writeFile(path.join(dir, ".env.example"), envTemplate(mode));
    writeFile(path.join(dir, "index.ts"), `import { GoBlink } from "@goblink/merchant-sdk";
...
async function main() {
    ...
    const framework = await ask("  Framework (1=Next.js, 2=Express, 3=None) [1]: ");
    const mode = await ask("  Mode (1=test, 2=live) [1]: ");
    const frameworkChoice = framework === "2" ? "express" : framework === "3" ? "none" : "nextjs";
    const modeChoice = mode === "2" ? "live" : "test";
    const dirName = frameworkChoice === "nextjs"
        ? "goblink-nextjs-app"
